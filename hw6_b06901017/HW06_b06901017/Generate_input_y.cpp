	...
	for(int i=0;i<2085;i++){
		...
		if(win()){//win immediately
			...
			if(sum2==9){//一開始下中間
				...
				//print_input_board();
				if(b[4]==0||b[4]==1){
					if(b[0]==0||b[0]==1) c[8]=1;
					if(b[2]==0||b[2]==1) c[6]=1;
					if(b[6]==0||b[6]==1) c[2]=1;
					if(b[8]==0||b[8]==1) c[0]=1;
					if(b[1]==0||b[1]==1) c[0]=1;
					if(b[3]==0||b[3]==1) c[0]=1;
					if(b[5]==0||b[5]==1) c[8]=1;
					if(b[7]==0||b[7]==1) c[8]=1;
				}
				if(b[4]==2){
